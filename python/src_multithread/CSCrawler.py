...
class CSCrawler(DataFetcher):
    ...
    def storeReviewsSqlite(self):
        print("Storing in db")
        print("{} para guardar".format(len(self.hosts)))
        try:
    ...
    def getReviewsThreaded(self):
        self.buffer_hosts = []
        lock = threading.Lock()
        t1 = threading.Thread(target=self.getReview,daemon=True,args=[lock])
        t2 = threading.Thread(target=self.getReview,daemon=True,args=[lock])
        t3 = threading.Thread(target=self.getReview,daemon=True,args=[lock])
        t4 = threading.Thread(target=self.getReview,daemon=True,args=[lock])
        ...
        self.hosts = self.buffer_hosts

    def getReview(self, lock):
        while len(self.hosts) > 0 :
            try:
                lock.acquire()
                host = self.hosts.pop()
                print("Hosts left {}".format(len(self.hosts)))
                lock.release()
                data = self.getSoup(u"https://www.couchsurfing.com" + host.endereco + "/references")
                campo_reviews = data.find("div", { "class" : "box-content mod-ovh"})
                if campo_reviews is not None:
